...
NORMAL=[('passing_touchdowns',r'passing (?:tds|touchdowns)'),('passing_yards',r'passing (?:yards|yds)'),('rushing_yards',r'rushing (?:yards|yds)'),('receiving_yards',r'receiving (?:yards|yds)'),('receptions',r'(?:^| )(?:total )?receptions(?: |$)'),('anytime_touchdown',r'anytime touchdown')]
...
def parse_runner(name,mt):
 ...
 if m:return m.group(1).strip(),m.group(2).lower(),(float(m.group(3)) if m.group(3) else None)
...
def extract(doc,state='VA',captured=None):
 ...
 for mid,m in markets.items():
  ...
  for r in runners:
   ...
   if not pn and re.match(r'^(Over|Under)\s+-?\d',rn,re.I): pn=re.sub(r'\s+-\s+(?:Alt )?(?:Passing|Rushing|Receiving|Total Receptions).*$', '', str(m.get('marketName','')), flags=re.I).strip()
   if line is None and mt in LINE_REQUIRED and isinstance(r.get('handicap'),(int,float)): line=float(r['handicap'])
   if not pn:
    skipped[mid]='cannot parse player';continue
   if mt in LINE_REQUIRED and line is None:
    skipped[mid]='required line absent';continue
