#!/usr/bin/env python3
"""CFB Week 4 one-time, owner-requested Sep 25 as-of rerun.

Uses the validated v2.2 feature stack and its historical fit, then advances 2026
team features through every completed game by appending a synthetic, outcome-null
as-of row per team. The previous scorer used the most recent *pregame* row for
prediction, silently omitting the most recent result. Existing already-started
games stay at their original pregame numbers. Book inputs remain comparison only.

Run only before Sep 25 Friday kickoff, with the verified Thursday Liberty-Coastal
boxscore on disk and current ESPN scoreboards available. This creates a distinct
final artifact; it does not replace the original full board or frozen initial file.
"""
...
last_completed['date']=pd.Timestamp('2026-09-25')
...
fp=REPO/'tools/cfb/17-cfb_board_2026_week4_full_v2_1.json'
frozen=json.load(open(fp)) if fp.exists() else {'games':[]}
frozen_by_id={g['event_id']:g for g in frozen['games'] if g['date'] < '2026-09-25T15:25Z'}
...
for eid,e in events.items():
    ...
    if eid in frozen_by_id:
        ...
        board.append({'event_id':eid,'name':e['name'],'date':comp['date'],'away':away,'home':home,'neutral_site':neutral,
            'timing':'pregame','timing_note':'Prior Week 4 pregame number retained for games already started; never retroactively rescored.',
    ...
    board.append({'event_id':eid,'name':e['name'],'date':comp['date'],'away':away,'home':home,'neutral_site':neutral,
        'timing':timing,'frozen_initial':False,
        'timing_note':('Week 4 final as-of Sep 25 before Friday kickoff; completed results through Liberty-Coastal included. Availability reports are linked on the page as context, without an unvalidated injury point adjustment.' if timing=='pregame' else 'Generated after kickoff - not a pregame number'),
...
# Carry forward previously verified TN/IL pre-kickoff quotes where today's IL feed has no row.
old_by_id={g['event_id']:g for g in frozen['games']}
for b in ok:
    if b.get('book_tn') or b.get('book_va'): continue
    prior=old_by_id.get(b['event_id'],{}).get('book_line')
    if prior and prior.get('state') in ('TN','IL','VA'):
        b['book_tn']=prior
...
out={'schema_version':'1.1.0','generated_at':gen,
 'model':'cfb-gameline-v2.2 (validated AP feature; Sep 25 as-of scoring-row correction)' ,
 'label':'MEASUREMENT ONLY - not picks. Week 4 final as-of model numbers vs state-labeled FanDuel book lines (display-only comparison). Model has not been proven against book lines. Totals side is a null and is omitted.',
 'window':'2026-09-24..2026-09-28 (Week 4, FBS)',
 'notes':['Final as-of numbers generated before Friday kickoff using completed results through Thursday. Historical training is unchanged; synthetic outcome-null as-of rows advance each 2026 team through its most recent final. Availability reports are linked as context only because injury is a NULL feature in the validated model. Book lines never enter the model.',
  'Book lines: FanDuel Illinois interim where available, TN fallback, state labeled per game. Display-only comparison, never a model input. Already-started games keep their original pregame number and last pre-kickoff book line.',
  'All not-yet-started games on this one-time final board are rescored; games already started retain original pregame numbers.',
  'Model: cfb-gameline-v2.2 adds validated point-in-time AP points difference to v2.1. AP experiment: validation MAE 13.504 vs 13.589 base; locked-test MAE 13.116 vs 13.360; bootstrap delta CI [-0.453,-0.040]. Current team stats include completed 2026 games through Thursday Liberty-Coastal.'],
 'games':ok,'errors':errs}
(OUT/'18-cfb_board_2026_week4_final_asof_v2_2.json').write_text(json.dumps(out,indent=1))
...
for b in ok:
    ...
    else: chips.append('<span class="chip chip-neutral">FINAL AS-OF SEP 25</span>')
...
<header><h1>E.D.I.T.H. <span>CFB</span> BOARD</h1><div id=updated>Week 4, 2026 - final as-of model + FanDuel book{(' - '+esc(BOOK_LABEL)) if BOOK_LABEL else ' (TN)'} - updated {esc(gen_label)}</div></header>
...
<div class=note-card><b>Book lines:</b> {esc(BOOK_LABEL) if BOOK_LABEL else 'FanDuel Tennessee, latest pre-kickoff capture per game'} - labeled comparison display, never a model input. MODEL display lines round to the nearest 0.5 point; underlying model outputs and grading precision are unchanged. These final as-of model numbers use completed results through Thursday and are fixed before Friday kickoff; only the book column updates. Comparison tracked, not a pick. CFB uses FBS sides/totals only, no college props.</div>
<div class=note-card><b>Availability context (not a model input):</b> No injury point adjustment was made. Check the latest <a href="https://www.secsports.com/fbreports">SEC availability reports</a> for Georgia and Tennessee/Texas, <a href="https://spartanswire.usatoday.com/story/sports/college/spartans/football/2026/09/23/michigan-state-nebraska-release-availability-reports-before-saturday/91912311007/">Michigan State-Nebraska reporting</a>, and <a href="https://superwestsports.com/utahs-availability-report-lists-three-questionable-for-iowa-state-game/">Utah-Iowa State reporting</a>. Status can change before kickoff; these sources are context, not an adjustment to model margins.</div>
<div class=note-card><b>Coverage:</b> {len(ok)} of {len(ok)+len(errs)} FBS-scheduled Week 4 games (Sep 24-28). All Saturday model numbers were rerun before Friday kickoff using completed results through Thursday; Friday games do not involve Saturday teams.</div>
<div class=note-card><b>Model:</b> <b>cfb-gameline-v2.2</b> - v2.1 statistical, roster-change, coach, NFL draft and 247Sports talent inputs, plus the validated point-in-time AP poll points difference from Sunday's experiment. AP candidate protocol: train 2020-23, validation 2024, locked test 2025; validation MAE 13.504 vs 13.589 base, locked-test MAE 13.116 vs 13.360, bootstrap delta 95% CI -0.453 to -0.040. Poll must be published strictly before kickoff; unranked teams are zero. Team stats include completed 2026 games through the Thursday Liberty-Coastal result and blend prior-season stats with in-season results at n/(n+2). <a href="https://github.com/Garrettwalker1/ediths-picks/tree/main/tools/model_cfb_v2">Artifact + backtest</a> - <a href="https://github.com/Garrettwalker1/ediths-picks/blob/main/tools/cfb/18-cfb_board_2026_week4_final_asof_v2_2.json">Raw board JSON</a> - <a href="https://github.com/Garrettwalker1/ediths-picks/blob/main/tools/model_cfb_v2/ap_poll_experiment.json">AP validation artifact</a></div>
