#!/usr/bin/env python3
"""Regenerate the CFB Week 3 board with FROZEN initial v2.1 numbers + FanDuel TN book overlay (display-only).
Usage: python3 score_full_week.py REPO_DIR OUT_DIR
- REPO_DIR: fresh clone of Garrettwalker1/ediths-picks
- OUT_DIR: writable output dir (writes 17-cfb_board_2026_week3_full_v2_1.json + cfb-board-2026-week3.html)
Every game ON the initial board keeps its frozen numbers VERBATIM from
tools/cfb/16-cfb_board_2026_week3_initial_v2_1.json - never rescored. Games added to the ESPN slate
...
never a model input. Usage: score_week3_full.py REPO_DIR OUT_DIR MERGED_TN_CSV 'label' TN
...
SLATE_DATES=['20260917','20260918','20260919','20260920','20260921']
...
JC=['team','pf_f','pa_f','yf_f','ya_f','pyf_r4','pya_r4','ryf_r4','rya_r4','tof_r4','qbc_r4','qby_r4','qbtd_r4','qbi_r4','gp','ret_share','portal_out_share','gone_share','portal_in_yds','qb_ret','new_coach','drafted_prod','talent']
gm=games.set_index('event_id').join(h[JC].add_prefix('h_'),how='inner').join(a[JC].add_prefix('a_'),how='inner').reset_index()
FEATS=[]
for s in ['pf_f','pa_f','yf_f','ya_f','pyf_r4','pya_r4','ryf_r4','rya_r4','tof_r4','qbc_r4','qby_r4','qbtd_r4','qbi_r4','ret_share','portal_out_share','gone_share','portal_in_yds','qb_ret','new_coach','drafted_prod','talent']:
...
feat26=pd.concat([s25[['team','pf_f','pa_f','yf_f','ya_f']+[c+'_r4' for c in r4cols]+['gp','ret_share','portal_out_share','gone_share','portal_in_yds','qb_ret','new_coach','drafted_prod','talent']],last26],ignore_index=True)
feat26=feat26.sort_values('gp').groupby('team').tail(1)
s25i=s25.set_index('team')
for c in ['pf_f','pa_f','yf_f','ya_f']+[c+'_r4' for c in r4cols]+['ret_share','portal_out_share','gone_share','portal_in_yds','qb_ret','new_coach','drafted_prod','talent']:
...
fp=REPO/'tools/cfb/16-cfb_board_2026_week3_initial_v2_1.json'
...
for eid,e in events.items():
    ...
    board.append({'event_id':eid,'name':e['name'],'date':comp['date'],'away':away,'home':home,'neutral_site':neutral,
        ...
        'timing_note':('ADDED POST-FREEZE: not on the initial board - scored at regen time, pregame.' if timing=='pregame' else 'Generated after kickoff - not a pregame number'),
...
out={'schema_version':'1.1.0','generated_at':gen,
 'model':'cfb-gameline-v2.1 (churn+coach+draft+247 talent composite)',
 'label':'MEASUREMENT ONLY - not picks. Frozen INITIAL model numbers vs FanDuel TN book lines (display-only comparison). Model has not been proven against book lines. Totals side is a null and is omitted.',
 'window':'2026-09-17..2026-09-21 (Week 3, FBS)',
 'notes':['Model numbers are the FROZEN initial model numbers from tools/cfb/16-cfb_board_2026_week3_initial_v2_1.json (frozen 2026-09-14 ~1:40 PM CT; book lines were never model inputs). They never move.',
  ...
  'Model: cfb-gameline-v2.1 (247Sports team-talent composite; locked-test 2025 MAE 12.94 vs 13.27, bootstrap CI clear). Team features blend prior-season stats with in-season 2026 results at weight n/(n+2); early-season MAE is higher (~15 vs ~13).'],
 'games':ok,'errors':errs}
(OUT/'17-cfb_board_2026_week3_full_v2_1.json').write_text(json.dumps(out,indent=1))
...
for b in ok:
    ...
    elif b.get('frozen_initial'): chips.append('<span class="chip chip-neutral">INITIAL</span>')
    else: chips.append('<span class="chip chip-flag">ADDED POST-FREEZE</span>')
    if bv and bv.get('home_spread') is not None:
        ...
        if b['timing']=='post_kickoff':
            ...
            gap='Not listed in any pre-kickoff FanDuel TN capture so far. The frozen initial number stands.'
    sec[b['week']].append(
...
wk_buttons=''.join(f'<button data-w="{w}"{ " class=on" if w==3 else ""}>WEEK {w}</button>' for w in sorted(sec, reverse=True))
wk_sections=''.join(f'<section data-week="{w}"{ "" if w==3 else " hidden"}>'+''.join(sec[w])+'</section>' for w in sorted(sec, reverse=True))
gen_label=now_ct.strftime('%Y-%m-%d %-I:%M %p CT')
page=f'''<!doctype html><html lang=en><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1">
<title>E.D.I.T.H. CFB model board - Week 3, 2026</title>
<style>
...
<nav>{wk_buttons}</nav>
{wk_sections}
<footer>E.D.I.T.H.\'s Picks - cfb-gameline-v2.1 - measurement, not picks. Book lines display-only, never model inputs.</footer>
...
(OUT/'cfb-board-2026-week3.html').write_text(page)
